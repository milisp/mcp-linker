...
curl -f -L \
  ...
  "https://api.github.com/repos/milisp/mcp-linker/releases/latest" > release.json
...
git clone https://x-access-token:${GITHUB_TOKEN}@github.com/milisp/homebrew-mcp-linker.git
cd homebrew-mcp-linker

# Determine the file to update
TARGET_FILE="Casks/mcp-linker.rb"

if [ ! -f "$TARGET_FILE" ]; then
  echo "Error: $TARGET_FILE not found in homebrew-mcp-linker repo"
  exit 1
fi
...
git add "$TARGET_FILE"
git commit -m "chore(cask): bump mcp-linker to ${VERSION}"
git push
